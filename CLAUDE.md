...
- **flickr/** - Flickr API client with OAuth 1.0a signing (`oauth.go`), HTTP helpers with retry/throttle (`http.go`), download logic (`download.go`), upload logic (`upload.go`, uses `filepath.WalkDir` for recursive subdirectory discovery), date resolution (`dates.go`), and metadata building (`metadata.go`). Uses transfer log files (`transfer.log`) for resumable downloads.
- **google/** - Google Photos API client with OAuth2 flow. `takeout.go` orchestrates a two-phase scan/extract approach for Google Takeout zip archives with context-aware cancellation. `takeout_scan.go` handles zip scanning, folder classification (album vs year folder), JSON sidecar matching, and deduplication decisions. `takeout_meta.go` handles JSON sidecar parsing (`photoTakenTime`, title, description). Metadata is embedded using the `jpegmeta`, `mp4meta`, and `xmp` packages. Uses upload log files (tracking by relative path) for resumable uploads.
- **s3/** - S3 operations via bundled rclone binary subprocess. `rclone.go` handles binary resolution (checks next to executable, then cwd) and temp config generation. `s3.go` builds rclone command args and runs them. `glacier.go` handles Glacier/Deep Archive detection and restore initiation. Upload defaults to Glacier Deep Archive (`--s3-storage-class DEEP_ARCHIVE`), configurable via `--storage-class` flag. Download detects Glacier objects, initiates Bulk restore (7-day availability), and reports pending restores — users re-run to download after restore completes.
...
- Resumable transfers: Flickr uses an append-only `transfer.log` for resumable downloads; Google Photos uses `.photo-copy-upload.log` for resumable uploads (tracked by relative path for subdirectory support). Failed files are not logged, so re-running retries them automatically. S3 relies on rclone's built-in diffing.
...
- The `--no-metadata` flag on `flickr download` and `google download` skips all metadata operations (XMP, MP4 creation time, filesystem timestamps).
...
- Google Takeout import uses a two-phase scan/extract approach: the scan phase builds an in-memory index with folder classification (album vs year folder), JSON sidecar matching, and dedup decisions; the extract phase writes files and embeds metadata. Context-awareness is preserved throughout: `extractFile` uses `copyWithContext` for cancellable large file copies, and the extract phase checks `ctx.Err()` to return immediately on cancellation.
