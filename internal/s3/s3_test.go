...
func TestRunRcloneWithProgress_GlacierPlusNonJSONFatal(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "segmentation fault") {
		t.Errorf("error should contain the fatal stderr text, got: %v", err)
	}
	if glacierPending != 1 {
		t.Errorf("glacierPending = %d, want 1", glacierPending)
	}
...
// TestClientDownload_GlacierRestoreTargeted verifies that backend restore
// only requests restore for Glacier files NOT already present locally.
func TestClientDownload_GlacierRestoreTargeted(t *testing.T) {
	workspace := t.TempDir()
	rcloneDir := filepath.Join(workspace, "tools-bin", "rclone")
	if err := os.MkdirAll(rcloneDir, 0755); err != nil {
		t.Fatal(err)
	...
	src := filepath.Join(t.TempDir(), "main.go")
	// Fake rclone that:
	// - lsf --format pT: returns two glacier files
	// - backend restore: reads --files-from and writes its contents to a marker
	// - copy: succeeds (no glacier errors since both are "restored")
	prog := `package main
...
func main() {
	for _, arg := range os.Args[1:] {
		if arg == "lsf" {
			for i, a := range os.Args {
				if a == "--format" && i+1 < len(os.Args) && os.Args[i+1] == "pT" {
					fmt.Fprintln(os.Stdout, "already-have.jpg;DEEP_ARCHIVE")
					fmt.Fprintln(os.Stdout, "need-restore.jpg;DEEP_ARCHIVE")
					os.Exit(0)
				}
			}
			fmt.Fprintln(os.Stdout, "already-have.jpg")
			fmt.Fprintln(os.Stdout, "need-restore.jpg")
			os.Exit(0)
		}
		if arg == "backend" {
			// Find --files-from and write its contents to a marker
			for i, a := range os.Args {
				if a == "--files-from" && i+1 < len(os.Args) {
					data, _ := os.ReadFile(os.Args[i+1])
					exe, _ := os.Executable()
					marker := filepath.Join(filepath.Dir(exe), "..", "..", "restore-files")
...
`
	...
	// Pre-create one of the glacier files — it should NOT appear in restore list
	if err := os.WriteFile(filepath.Join(outputDir, "already-have.jpg"), []byte("data"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	_, err := client.Download(context.Background(), "test-bucket", "prefix", outputDir, false, 0, nil)
	if err != nil {
		t.Fatalf("Download failed: %v", err)
	}

	// Verify restore was called with only the missing file
	markerPath := filepath.Join(workspace, "restore-files")
	data, err := os.ReadFile(markerPath)
	if err != nil {
		t.Fatal("backend restore was not invoked (marker file not found)")
	}
	contents := strings.TrimSpace(string(data))
	if contents != "need-restore.jpg" {
		t.Errorf("restore --files-from should contain only 'need-restore.jpg', got: %q", contents)
	}
}
